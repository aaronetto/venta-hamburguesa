...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre)) {
        ...
        // Verificar si ya existe una provincia con ese nombre en la misma ciudad
        $check_query = "SELECT COUNT(*) as total FROM provincia WHERE NOMBRE = ? AND ID_CIUDAD = ?";
        ...
        if ($check_row['total'] > 0) {
            $error = 'Ya existe una provincia con ese nombre en la ciudad seleccionada.';
        } else {
            // Insertar la nueva provincia
            $insert_query = "INSERT INTO provincia (NOMBRE, ID_CIUDAD) VALUES (?, ?)";
            $insert_stmt = $conexion->prepare($insert_query);
            $insert_stmt->bind_param("si", $nombre, $id_ciudad);
            
            if ($insert_stmt->execute()) {
                header('Location: index.php?success=provincia_creada');
                exit();
            } else {
                $error = 'Error al registrar la provincia.';
            }
        }
    }
}
...
<head>
    ...
    <title>Registrar Provincia - Administrador</title>
...
<body>
    <div class="admin-container">
        ...
        <div class="admin-header">
            <h1 class="admin-title">🏛️ Registrar Provincia</h1>
        </div>
        
        <div class="form-container">
            <?php if ($ciudades_result->num_rows === 0): ?>
                <div class="no-ciudades">
                    <strong>⚠️ No hay ciudades registradas</strong><br>
                    Debe registrar al menos una ciudad antes de crear provincias.
                    <br><br>
                    <a href="../ciudades/registrar.php" style="color: #007bff; text-decoration: none;">
                        → Registrar Ciudad
                    </a>
                </div>
            <?php else: ?>
                ...
                <form method="POST" action="">
                        ...
                        <select id="id_ciudad" 
                            ...
                            <?php while ($ciudad = $ciudades_result->fetch_assoc()): ?>
                                <option value="<?php echo $ciudad['ID_CIUDAD']; ?>"
                                        <?php echo (isset($_POST['id_ciudad']) && $_POST['id_ciudad'] == $ciudad['ID_CIUDAD']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-buttons">
                        <button type="submit" class="btn-primary">Registrar Provincia</button>
